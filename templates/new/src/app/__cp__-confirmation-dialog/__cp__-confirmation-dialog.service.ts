...
export class __capcp__ConfirmationDialogService {
  ...
  ): Observable<__capcp__ConfirmationDialogComponent> {
    const title = 'Fehler';
    const buttons: Array<ButtonData> = [
      new ButtonData('CANCEL', 'Schliessen', 'warn'),
    ];
    const data: ConfirmationData = new ConfirmationData(
      title,
      phrases,
      buttons
    );
  ...
  public simpleConfirm(
    ...
    const dlg = this.dialog.open(__capcp__ConfirmationDialogComponent, {
      height: data.height,
      width: data.width,
      data,
      disableClose: false,
