...
Feature: 001 Login
  ...
  Scenario: Home Page
    ...
    When I do nothing
    Then I should see the page title "Login"

  @login @happy
  Scenario: The login button should be disabled when I enter the login page
    Given I am on the "login" page
    Then The button "login-login-button" should be disabled
  ...
  Scenario: The login button should be enabled when user/password is long enough
    ...
    Then The button "login-login-button" should be enabled
...
#      And I expect errors in the logfile
    ...
    When I click the button "login-login-button"
