...
Given(/^I am logged in$/, { timeout: 20 * 1000 }, async () => {
...
Given(/^I am logged in as Kontingentkunde$/, { timeout: 20 * 1000 }, async () => {
	await page.navigateTo('anmelden');
...
Given(
	/^The radio button "([^"]*)" is selected in group "([^"]*)"$/,
	{ timeout: 20 * 1000 },
...
Given(/^The input field "([^"]*)" is empty$/, { timeout: 20 * 1000 }, async (field: string) => {
...
Given(/^I am on the "([^"]*)" page$/, { timeout: 10 * 1000 }, async (path: string) => {
	await page.navigateTo(path);
});

Given(/^I expect errors in the logfile$/, { timeout: 20 * 1000 }, async () => {
...
When(/^I do nothing$/, { timeout: 20 * 1000 }, () => {});

When(
	/^I enter "([^"]*)" with RETURN in the input field "([^"]*)"$/,
	{ timeout: 20 * 1000 },
...
When(
	/^I remove the input of the field "([^"]*)"$/,
	{ timeout: 20 * 1000 },
	async (field: string) => {
		await page.clearInput(field);
	}
...
When(/^I drag "([^"]*)" to "([^"]*)"$/, { timeout: 20 * 1000 }, async (id1, id2: string) => {
...
When(
	/^I remove the input of the autocomplete field "([^"]*)"$/,
	{ timeout: 20 * 1000 },
	async (field: string) => {
		await page.clearInput(field);
	}
...
When(/^I click the button "([^"]*)"$/, { timeout: 20 * 1000 }, async (id: string) => {
	await page.pressButtonById(id); // old way
});

When(/^I click the radio button "([^"]*)"$/, { timeout: 20 * 1000 }, async (id: string) => {
	const radioButton = await page.getRadioButton(id);
	await radioButton.check();
});

When(/^I click the slide toggle "([^"]*)"$/, { timeout: 20 * 1000 }, async (id: string) => {
	const slide = await page.getSlideToggle(id);
	await slide.toggle();
});

When(/^I click the link "([^"]*)"$/, { timeout: 20 * 1000 }, async (id: string) => {
	await page.pressAnchorById(id);
});

When(/^I wait (\d+)ms$/, { timeout: 20 * 1000 }, async (waitMs: string) => {
...
Then(
	/^The select box "([^"]*)" should be set to "([^"]*)"$/,
	{ timeout: 20 * 1000 },
...
Then(/^I should see the page title "([^"]*)"$/, { timeout: 20 * 1000 }, async (title: string) => {
	await browser.sleep(1000);
	expect(await page.getTitleText()).to.equal(title);
});

Then(
	/^The button "([^"]*)" should be (enabled|disabled)$/,
	{ timeout: 20 * 1000 },
...
Then(
	/^The input "([^"]*)" should be (enabled|disabled)$/,
	{ timeout: 20 * 1000 },
	async (id: string, state: string) => {
		if (state === 'enabled') {
...
Then(
	/^The (shadow |)input "([^"]*)" should contain the text "([^"]*)"$/,
	{ timeout: 20 * 1000 },
...
Then(/^The input "([^"]*)" should be empty$/, { timeout: 20 * 1000 }, async (id: string) => {
	expect(await page.getInputValue(id)).to.equal('');
});

Then(
	/^The fieldset "([^"]*)" should be (enabled|disabled)$/,
	{ timeout: 20 * 1000 },
	async (id: string, state: string) => {
...
Then(
	/^The radio button "([^"]*)" should be (checked|unchecked)$/,
	{ timeout: 20 * 1000 },
	async (id: string, state: string) => {
		const radioButton = await page.getRadioButton(id);
		if (state === 'checked') {
...
Then(
	/^The radio button "([^"]*)" should be (enabled|disabled)$/,
	{ timeout: 20 * 1000 },
	async (id: string, state: string) => {
		const radioButton = await page.getRadioButton(id);
		if (state === 'enabled') {
...
Then(
	/^The slide toggle "([^"]*)" should be (on|off)$/,
	{ timeout: 20 * 1000 },
	async (id: string, state: string) => {
		if (state === 'on') {
...
Then(
	/^The select box "([^"]*)" should be (enabled|disabled)$/,
	{ timeout: 20 * 1000 },
	async (id: string, state: string) => {
		const select = await page.getSelect(id);
		if (state === 'enabled') {
...
Then(/^The select box "([^"]*)" should be empty$/, { timeout: 20 * 1000 }, async (id: string) => {
	const select = await page.getSelect(id);
	expect((await select.getValueText()).trim()).to.equal('');
});

Then(
	/^I should see an error message containing "([^"]*)" displayed$/,
	{ timeout: 20 * 1000 },
	async (text: string) => {
...
Then(
	/^The specific (hint|error) message "([^"]*)" should contain "([^"]*)"$/,
	{ timeout: 20 * 1000 },
...
Then(
	/^The specific element "([^"]*)" should be missing$/,
	{ timeout: 20 * 1000 },
	async (id: string) => {
...
Then(
	/^The specific element "([^"]*)" should exist$/,
	{ timeout: 20 * 1000 },
	async (id: string) => {
		const ele = await element(by.deepCss(`[data-test-id=${id}]`));
...
Then(
	/^I should see a dialog with the title containing "([^"]*)"$/,
	{ timeout: 20 * 1000 },
	async (title: string) => {
...
Then(/^I should see no dialog$/, { timeout: 20 * 1000 }, async () => {
	expect((await page.getDialogCount()).length).to.equal(0);
});

Then(
	/^I should see the text "([^"]*)" in the sidenav$/,
	{ timeout: 20 * 1000 },
	async (text: string) => {
...
Then(
	/^The option-menu "([^"]*)" should be set to "([^"]*)"$/,
...
After(async function (scenario) {
	...
	if (page.isChrome) {
		if (scenario.result.status === Status.FAILED || scenario.result.status === Status.AMBIGUOUS) {
					...
					'<pre>' +
						logs
							.map((logentry, index) => {
								...
								return `${prefix ? prefix : ''}${logentry.timestamp} ${logentry.level.name} ${
									logentry.message
								}}${prefix ? '</span>' : ''}`;
							})
							.join('\r\n') +
