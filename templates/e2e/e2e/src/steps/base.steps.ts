...
Given(
  /^I am logged in as Kontingentkunde$/,
  { timeout: 20 * 1000 },
  async () => {
    await page.navigateTo('anmelden');
...
Then(
  /^The select box "([^"]*)" should be empty$/,
  { timeout: 20 * 1000 },
  async (id: string) => {
    const select = await page.getSelect(id);
...
Then(
  ...
  { timeout: 20 * 1000 },
  async (title: string) => {
    expect((await page.getDialogCount()).length, 'too many dialogs!').to.equal(
      1
    );
              ...
              .map((logentry, index) => {
                ...
                if (page.isIgnorableError(logentry)) {
                  ...
                  prefix = `<span style="color: ${
                    page.expectSevereLogEntries ? 'green' : 'red'
                  }">`;
                }
                return `${prefix ? prefix : ''}${logentry.timestamp} ${
                  logentry.level.name
                } ${logentry.message}}${prefix ? '</span>' : ''}`;
              })
              .join('\r\n') +
            '</pre>'
        ),
