...
export class AppPage {
	...
	getCheckbox(id: string) {
		return this.loader.getHarness(MatCheckboxHarness.with({ selector: `[data-test-id=${id}]` }));
	}

	getOptionMenuValue(id: string) {
		return element.all(by.deepCss(`mat-select[data-test-id=${id}]`)).map(x => x.getText());
	}
	...
	getOptionsShadowDom() {
		return element.all(by.deepCss('span.mat-option-text')).map(x => x.getText()) as Promise<
			string[]
		>;
	}
	...
	hasErrorsInLog(entries: logging.Entry[]) {
		return (
			entries.filter(
				logEntry => logEntry.level === logging.Level.SEVERE && !this.isIgnorableError(logEntry)
			).length > 0
		);
	}
