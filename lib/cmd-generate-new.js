...
const {changePackageJsonScripts, changePackageJson} = require("./tweak-package-json");
...
const generateNewProject = async (options) => {
  ...
  const initCmds = [
    ...
    'npm i jest@25.0.0 jest-preset-angular @types/jest@^25.2.2 --save-dev',
    `copy templates`,
    'extend package.json',
    'extend tsconfig.spec.json',
    'delete test.ts',
    'change logo',
    `fix files`,
    `rename files`
  ...
  if (confirmed) {
    ...
    for (let i = 0; i < cmds.length; i++) {
      ...
      } else if (cmd === 'extend tsconfig.spec.json') {
        const tscsj = './' + appFolder + '/tsconfig.spec.json';
        let jsonObject = fse.readJsonSync(tscsj);
        const o = {
          "emitDecoratorMetadata": true,
          "esModuleInterop": true
        };
        jsonObject.compilerOptions = {...jsonObject.compilerOptions, ...o};
        fse.writeJsonSync(tscsj, jsonObject, {spaces: 2});
      ...
      } else if (cmd === 'delete test.ts') {
        await fse.removeSync('./' + appFolder + '/src/test.ts');
